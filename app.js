...
const mainCategories = [
  { id: 'all', name: '전체 도서 보기' },
  { id: 'literature', name: '문학' },
  { id: 'nonfiction', name: '비문학' }
];
...
function isBookInLibrary(recBook) {
  return state.books.some(
    b => b.title.trim().toLowerCase() === recBook.title.trim().toLowerCase() &&
         b.author.trim().toLowerCase() === recBook.author.trim().toLowerCase()
  );
}
...
function renderProfileSetup() {
  ...
  if (saved) {
    try {
      ...
      // Ignored if profiles data is malformed
    }
  }
    ...
    btn.addEventListener('click', () => {
      ...
      if (saved) {
        try {
          ...
          // Ignored if login session lookup fails
        }
      }
    });
  });
